...
- **Automated tests**: 17 unit tests via `cargo test`, covering the sanitizer (script stripping, `javascript:` URLs, non-YouTube iframe removal, size cap), the validators (user id charset, UUID document ids, permission values), the handler helpers, a 34-payload injection corpus, and policy guards that fail the build if the sanitizer allow-list or the CSP is widened. An earlier AI-written integration test file was deleted rather than kept green-by-luck: it required a live MongoDB and did not compile.
